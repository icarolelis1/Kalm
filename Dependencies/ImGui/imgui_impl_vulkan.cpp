...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer,VK_Objects::CommandPool& pool,const VK_Objects::Device device)
{
    ...
    VK_Objects::Buffer buffer(&device, upload_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);

    // Upload to Buffer:
    {
        char* map = NULL;
        err = vkMapMemory(v->Device, buffer.getMemoryHandle(), 0, upload_size, 0, (void**)(&map));
        check_vk_result(err);
        memcpy(map, pixels, upload_size);
        VkMappedMemoryRange range[1] = {};
        range[0].sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
        range[0].memory = buffer.getMemoryHandle();
        range[0].size = upload_size;
        err = vkFlushMappedMemoryRanges(v->Device, 1, range);
        check_vk_result(err);
        vkUnmapMemory(v->Device, buffer.getMemoryHandle());
    }
    ...
    // Copy to Image:
    {
        ...
        */
        Vk_Functions::setImageLayout(device, pool, bd->FontImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    ...
    //Vk_Functions::copyBufferToImage(.)
        ...
        */
        Vk_Functions::setImageLayout(device, pool, bd->FontImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
